...
from PhotoShare.src.database import get_db
from PhotoShare.src.services import auth_service
from PhotoShare.src.services import roles

from PhotoShare.src.models.user import User !!!!!!!!

from PhotoShare.src.schemas.comment import CommentModel, CommentResponse

from PhotoShare.src.repositories import comments as repository_comments
from PhotoShare.src.repositories import photo as repository_photos

router_comments = APIRouter(prefix='/comments', tags=["comments"])


@router_comments.get("/", response_model=List[CommentResponse])
async def read_comments(limit: int = 100, photo_id: int = 0, db: Session = Depends(get_db)):
    """
...
async def create_comment(body: CommentModel, photo_id: int = 0, db: Session = Depends(get_db),
